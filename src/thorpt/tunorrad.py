...
def run_theriak(database, temperature, pressure, whole_rock):
    ...
    if platform.system() == 'Windows':
        data_folder = main_folder / "_theriak" / "WIN"
...
def read_theriak(database, temperature, pressure, whole_rock):
    ...
    # ANCHOR time start normal
    print("Normal warpping script")
    start = time.time()
    ...
    volume_density_index = int(theriak_in_lines.index(keyword))
    ...
    # try:
    if keyword in theriak_in_lines:
        volume_density_values_fluids = int(theriak_in_lines.index(keyword))
        gases_fluids = True
    # except ValueError:
    else:
        ...
        while current_number <= 10:
            try:
                vol_dens_solid = decode_lines(
                    volume_density_values_solids + current_number, theriak_in_lines, number=200
                )
            except IndexError:
                break
    ...
    try:
        theriak_in_lines[volume_density_values_fluids] == fluid_check
        index_list = ['N', 'volume/mol', 'volume[ccm]',
                      'wt/mol', 'wt[g]', 'density[g/ccm]']
        df_Vol_Dens_fluids = read_to_dataframe(
            index_entry=volume_density_values_fluids, df_index_as_list=index_list, input_in_lines=theriak_in_lines)
    ...
    try:
        fluid_content_solids_index = int(theriak_in_lines.index(keyword)) + 3
        hydrous_solid = True
    ...
    try:
        ...
        df_H2O_content_solids = read_to_dataframe(
            index_entry=fluid_content_solids_index, df_index_as_list=index_list, input_in_lines=theriak_in_lines)
    ...
    try:
        h2o_content_fluids_index = int(theriak_in_lines.index(keyword))
    ...
    try:
        ...
        df_H2O_content_fluids = read_to_dataframe(
            index_entry=h2o_content_fluids_index, df_index_as_list=index_list, input_in_lines=theriak_in_lines)
    except NameError:
    ...
    try:
        elements_stable_index = int(theriak_in_lines.index(keyword))
    except ValueError:
        print("---ERROR:---Where are my elements???")
    # need to define index list by decoding the output first

    temp = theriak_in_lines[elements_stable_index +
                            3] + theriak_in_lines[elements_stable_index+4]
    ...
    if one_element_row is False:
        while True:
            if theriak_in_lines[elements_stable_index + 4 + current_number] == ' elements per formula unit:':
                break
    ...
    if one_element_row is True:
        count = 0
        while True:
            if 'total' in theriak_in_lines[elements_stable_index+4+count]:
                el_phase = theriak_in_lines[elements_stable_index +
                                            4+count].split()
                for i, item in enumerate(el_phase[1:]):
                    el_phase[i+1] = float(item)
                data[el_phase[0]] = pd.Series(el_phase[1:])
                break
            el_phase = theriak_in_lines[elements_stable_index+4+count].split()
            for i, item in enumerate(el_phase[1:]):
                el_phase[i+1] = float(item)
            data[el_phase[0]] = pd.Series(el_phase[1:])
            count += 1
    ...
    eq_ass_index = int(theriak_in_lines.index(keyword)) + 12
    ...
    numb_p = int(theriak_in_lines[int(
        theriak_in_lines.index(keyword)) + 5].split()[2])
    temp2 = theriak_in_lines[theriak_in_lines.index(
        keyword) + 7].split('     ')
    ...
    while eq_ass_index < stop:
        ...
        temp = theriak_in_lines[eq_ass_index].split()
    ...
    for num, item in enumerate(phase_pos):
        ...
        temp = theriak_in_lines[item].split()
        if temp[2] == 'GARNET':
            break
        if '**' in temp:
            temp.remove('**')
        ...
        if len(temp) > 5:
            ...
            while check_solid_solutions is True:
                endmember += 1
                # print(theriak_in_lines[item+endmember])
                ass_cache = theriak_in_lines[item+endmember].split()
                if '**' in ass_cache:
                    ass_cache.remove('**')
                if not ass_cache:
                    break
    ...
    i_base = int(theriak_in_lines.index(keyword))
    n_elem = True
    pot_index = i_base + 12
    ...
    while n_elem is True:
        test = theriak_in_lines[pot_index+i]
        if test == ' ':
            n_elem = i
        i += 1

    pot_col = theriak_in_lines[pot_index].split()
    pot_d = theriak_in_lines[pot_index+2:pot_index+n_elem]
    pot_el = []
    pot_val = []
    ...
    print(f"Script Normal - Time taken: {(end-start)*10**3:.09f}ms")
    """
